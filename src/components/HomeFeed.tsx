...
export default function HomeFeed({
  ...
  const handleCreatePost = () => {
    ...
    onAddPost({
      ...
      taggedMembers: newPost.taggedMembers.length > 0 ? newPost.taggedMembers : undefined
    })

    setNewPost({ content: '', media: [], location: '', taggedMembers: [] })
  ...
  const navigateMedia = (postId: string, direction: 'prev' | 'next', totalMedia: number) => {
    setMediaIndex(prev => {
      ...
      const next = direction === 'next'
        ...
        : (current - 1 + totalMedia) % totalMedia
    ...
    <div className="space-y-6 max-w-2xl mx-auto">
      ...
      <div className="card-glass p-5 animate-slide-up">
        <div className="flex items-center gap-4">
          <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-lavender-400 to-lavender-500 flex items-center justify-center flex-shrink-0 shadow-soft">
      ...
      <div className="space-y-6">
        {posts.map((post, index) => {
          ...
          return (
            ...
            >
              {/* Pinned Gift Banner */}
              {post.isPinned && post.linkedGiftId && (
                <div className="bg-gradient-to-r from-lavender-50 to-peach-50 px-5 py-3 flex items-center justify-between border-b border-lavender-100/50">
                  <div className="flex items-center gap-2">
                    <Gift className="h-5 w-5 text-lavender-500 animate-float-slow" />
                    <span className="text-sm font-medium text-lavender-700">Family Fun Announcement</span>
                  </div>
                  {onViewGift && (
                    <button
                      onClick={() => onViewGift(post.linkedGiftId!)}
              ...
              <div className="p-5 pb-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="h-11 w-11 rounded-2xl bg-gradient-to-br from-lavender-400 to-mint-400 flex items-center justify-center shadow-soft">
                  ...
                  <button className="p-2 hover:bg-cream-100 rounded-xl transition-colors">
                    <MoreHorizontal className="h-5 w-5 text-navy-400" />
                  </button>
                </div>
              </div>
              ...
              {post.media && post.media.length > 0 && (
                <div className="relative img-zoom">
                  ...
                  {hasMultipleMedia && (
                    <>
                      <button
                        onClick={() => navigateMedia(post.id, 'prev', post.media!.length)}
                      ...
                      <button
                        onClick={() => navigateMedia(post.id, 'next', post.media!.length)}
                        ...
                        {post.media.map((_, i) => (
                          <div
                            ...
                            className={`h-2 rounded-full transition-all ${
                              i === currentMediaIndex ? 'bg-white w-5' : 'bg-white/50 w-2'
                            }`}
                          />
                        ))}
                      </div>
              ...
              <div className="px-5 py-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1">
                    ...
                    >
                      <Heart
                        className={`h-5 w-5 transition-all ${isLiked ? 'fill-peach-500 animate-heart-pop' : ''}`}
                      />
                ...
                {post.likes.length > 0 && (
                  <div className="mt-3 text-sm text-navy-600">
                    ...
                    <span className="font-medium text-navy-800">
                      {post.likes.slice(0, 2).map(id =>
                        members.find(m => m.id === id)?.name.split(' ')[0] || 'Someone'
                      ).join(', ')}
                ...
                {(showComments || post.comments.length > 0) && (
                  <div className="mt-4 pt-4 border-t border-lavender-100/50">
                    ...
                    {!showComments && post.comments.length > 2 && (
                      <button
                        onClick={() => toggleComments(post.id)}
                        className="text-sm text-lavender-600 hover:text-lavender-700 font-medium mb-3"
                      >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 flex items-center gap-2 bg-cream-100/80 rounded-2xl px-4 py-2 border border-cream-200">
                        ...
                        />
                        <button className="p-1 hover:bg-cream-200 rounded-lg transition-colors">
                          <Smile className="h-5 w-5 text-navy-400" />
                        </button>
                        <button
        ...
        <div className="fixed inset-0 bg-navy-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-fade-in">
          <div className="bg-white rounded-3xl max-w-lg w-full shadow-glass-lg animate-scale-in max-h-[90vh] overflow-y-auto">
            <div className="p-5 border-b border-lavender-100/50 flex items-center justify-between sticky top-0 bg-white rounded-t-3xl z-10">
              <h3 className="text-xl font-bold text-navy-900">Share a Moment</h3>
              <button
            ...
            <div className="p-5 space-y-4">
              {/* Author */}
              <div className="flex items-center gap-3">
                <div className="h-12 w-12 rounded-2xl bg-gradient-to-br from-lavender-400 to-lavender-500 flex items-center justify-center shadow-soft">
                ...
                <div>
                  ...
                  <p className="text-sm text-navy-500">Sharing with family</p>
                </div>
              </div>
                  ...
                  {newPost.media.map((item, i) => (
                    <div key={i} className="relative rounded-2xl overflow-hidden aspect-square">
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 p-1.5 bg-navy-900/70 hover:bg-navy-900 rounded-full transition-colors"
                      >
              ...
              {/* Tag Members */}
              <div>
                <p className="text-sm font-medium text-navy-700 mb-2">Tag family members</p>
                  ...
                  {members.filter(m => m.id !== currentUserId).map(member => (
                    <button
                      ...
                      onClick={() => {
                        setNewPost(prev => ({
                          ...prev,
                          taggedMembers: prev.taggedMembers.includes(member.id)
                            ? prev.taggedMembers.filter(id => id !== member.id)
                            : [...prev.taggedMembers, member.id]
                        }))
                      }}
                      className={`px-3 py-1.5 rounded-xl text-sm font-medium transition-all ${
                        newPost.taggedMembers.includes(member.id)
            ...
            <div className="p-5 border-t border-lavender-100/50 flex items-center justify-between sticky bottom-0 bg-white rounded-b-3xl">
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="p-3 hover:bg-mint-50 rounded-xl transition-colors"
                >
                  <ImageIcon className="h-6 w-6 text-mint-500" />
                </button>
                <button
                  onClick={() => fileInputRef.current?.click()}
                  className="p-3 hover:bg-lavender-50 rounded-xl transition-colors"
                >
