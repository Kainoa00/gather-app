...
export default function CareCircle({ members, onAddMember }: CareCircleProps) {
  ...
  const generateInviteLink = () => {
    const link = `https://parentplanner.app/invite/${Math.random().toString(36).substring(7)}`
    setInviteLink(link)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Care Circle</h2>
          ...
          {Object.entries(roleConfig).map(([key, config]) => {
            ...
            return (
              <div key={key} className="flex items-center">
                ...
                <span className="ml-2 text-sm text-gray-500">{config.description}</span>
              </div>
            )
          })}
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-xl">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-semibold text-gray-900">Invite to Care Circle</h3>
              <button
